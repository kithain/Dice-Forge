...
def validate_encounter_name(name):
    """Retourne un message d'erreur si le nom ne peut pas devenir un fichier Windows."""
    if not isinstance(name, str) or not name:
        return "Le nom du combat est obligatoire."
    if name != name.strip():
        return "Le nom ne peut pas commencer ou finir par un espace."
    if len(name) > ENCOUNTER_NAME_MAX_LENGTH:
        return f"Le nom du combat est limité à {ENCOUNTER_NAME_MAX_LENGTH} caractères."
    if any(character in WINDOWS_INVALID_FILENAME_CHARS or ord(character) < 32 for character in name):
        return 'Caractère interdit dans un nom de fichier Windows : < > : " / \\ | ? *'
    if name.endswith('.'):
        return "Le nom ne peut pas se terminer par un point."
    if name in {".", ".."}:
        return "Ce nom de fichier est interdit."
...
def save_players(initiative_data):
    ...
    try:
        ...
        print(f"[ERREUR] Impossible de sauvegarder les joueurs: {e}")
        return False
...
def save_encounter(name, initiative_data):
    ...
    Returns:
        str|None: Le nom du fichier où la rencontre a été sauvegardée, ou None en cas d'erreur.
    """
    if validate_encounter_name(name):
        return None
    try:
        encounter = {
            'name': name,
        ...
        print(f"[ERREUR] Impossible de sauvegarder la rencontre '{name}': {e}")
        return None
...
def delete_encounter(filename):
    """Supprime une sauvegarde de rencontre JSON située dans ENCOUNTERS_DIR."""
    if not filename or os.path.basename(filename) != filename or not filename.lower().endswith('.json'):
        return False
    encounters_root = os.path.realpath(ENCOUNTERS_DIR)
    target = os.path.realpath(os.path.join(encounters_root, filename))
    try:
        if os.path.commonpath([encounters_root, target]) != encounters_root:
            return False
    except ValueError:
        return False
    ...
    try:
        ...
        print(f"[ERREUR] Impossible de supprimer la rencontre '{filename}': {error}")
        return False
