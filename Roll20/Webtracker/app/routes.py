...
def _tracker_context():
    models.ensure_participant_colors(models.combat.participants)
    return {
...
def edit_participant(p_index):
    ...
    participant.set_hp(_form_int("hp", participant.hp, 0))
...
@app.route("/restart_combat", methods=["POST"])
def restart_combat():
    models.combat.restart()
    return jsonify({"success": True})
...
def save_encounter_route():
    name = request.form.get("encounter_name") or ""
    validation_error = utils.validate_encounter_name(name)
    if validation_error:
        return jsonify({"success": False, "message": validation_error}), 400
    filename = utils.save_encounter(name, models.combat.participants)
    return jsonify({"success": bool(filename), "message": "Sauvegarde impossible." if not filename else ""})
...
@app.route("/delete_encounter/<filename>", methods=["POST"])
def delete_encounter_route(filename):
    """Supprime une sauvegarde de combat, sans toucher au combat en cours."""
    deleted = utils.delete_encounter(filename)
    if not deleted:
        return jsonify({"success": False, "message": "Combat introuvable ou chemin invalide."}), 404
