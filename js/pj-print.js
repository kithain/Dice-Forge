...
function renderPrintSheet(data) {
  ...
  const weapons = (data.weapons || []).filter(weapon => ['name', 'damage'].some(key => String(weapon[key] || '').trim()));
  const weaponRows = weapons.length ? weapons.map(weapon => `<tr><td>${text(weapon.name)}</td><td>${weapon.attackType === 'mixed' ? 'Contact + jet' : weapon.attackType === 'distance' ? 'Jet' : weapon.attackType === 'contact' ? 'Contact' : '—'}</td><td>${text(weapon.contactScore)}</td><td>${text(weapon.distanceScore)}</td><td>${text(weapon.damage)}</td></tr>`).join('') : '<tr><td colspan="5">Aucune arme renseignée</td></tr>';
  ...
  document.getElementById('print-sheet').innerHTML = `
    ...
    <div class="page-break">
      ...
      <table><thead><tr><th>Arme</th><th>Classe</th><th>% contact</th><th>% jet</th><th>Dégâts</th></tr></thead><tbody>${weaponRows}</tbody></table>
